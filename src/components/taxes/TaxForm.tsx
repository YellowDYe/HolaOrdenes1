...
interface TaxFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateTaxData) => Promise<void>;
  editingTax?: Tax | null;
  loading?: boolean;
}
...
const TaxForm: React.FC<TaxFormProps> = ({
  ...
  // Reset form when modal opens/closes or editing tax changes
  useEffect(() => {
    if (isOpen) {
      if (editingTax) {
        setFormData({
          tax_name: editingTax.tax_name,
          tax_percentage: editingTax.tax_percentage
        });
      } else {
        setFormData({
          tax_name: '',
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.tax_name.trim()) {
      newErrors.tax_name = 'El nombre del impuesto es requerido';
    } else if (formData.tax_name.trim().length < 2) {
      newErrors.tax_name = 'El nombre debe tener al menos 2 caracteres';
    } else if (formData.tax_name.trim().length > 100) {
      newErrors.tax_name = 'El nombre no puede exceder 100 caracteres';
    }

    if (formData.tax_percentage < 0) {
      newErrors.tax_percentage = 'El porcentaje no puede ser negativo';
    } else if (formData.tax_percentage > 100) {
      newErrors.tax_percentage = 'El porcentaje no puede exceder 100%';
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900 font-poppins">
            {editingTax ? 'Editar Impuesto' : 'Agregar Impuesto'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-all ${
                  errors.tax_name ? 'border-red-500' : 'border-gray-300'
                }`}
            ...
            {errors.tax_name && (
              <p className="mt-1 text-sm text-red-600">{errors.tax_name}</p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-all ${
                errors.tax_percentage ? 'border-red-500' : 'border-gray-300'
              }`}
              placeholder="16.00"
              disabled={loading}
            />
